...
```
 3.3. 静态页面
 Laravel 教程 - Web 开发实战入门 ( Laravel 5.1 ) /  静态页面
...
在上面代码的代码中，我们为 get 方法传递了两个参数，第一个参数指明了 URL，第二个参数指明了处理该 URL 的控制器动作。get 表明这个路由将会响应 GET 请求，并将请求映射到指定的控制器动作上。比方说，我们向 http://sample.test/ 发出了一个请求，则该请求将会由 StaticPagesController 的 home 方法进行处理。我们将在下节创建 StaticPagesController，为你讲解控制器在收到请求后如何进行相关操作。
...
make:controller 命令将会为我们生成一个控制器，该控制器默认为我们定义好一些常用的动作，如 index, create, store 等。但这些动作我们在静态页面控制器中都不会用到，且生成的动作代码对新手不友好，新手难以理解生成方法的具体作用。因此我们可通过在该命令后面加上 --plain 选项来生成更加简洁的控制器。

如果你运行了前面生成静态页面控制器的命令，可以先瞧一瞧生成的 StaticPagesController 文件里面的内容，再把删掉：

$ rm app/Http/Controllers/StaticPagesController.php
接下来让我们加上 --plain 参数重新创建静态页面控制器：

$ php artisan make:controller StaticPagesController --plain
从 Laravel 5.2 起，--plain 被移除，在创建控制器时默认不会再生成其它动作。
